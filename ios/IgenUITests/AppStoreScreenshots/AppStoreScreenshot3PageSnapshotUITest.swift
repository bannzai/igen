...
/// App Store スクリーンショット 3 枚目 (AppStoreScreenshot3Page) を全言語で撮影する。
/// Attachment 名 `{クラス名}---{関数名}---{言語}---{index}` は scripts/generate_screenshots/organize_appstore_screenshots.sh が解析する
final class AppStoreScreenshot3PageSnapshotUITest: XCTestCase {
  ...
  let previewType = "AppStoreScreenshot3Page_Previews"
  let previewCount = 1
