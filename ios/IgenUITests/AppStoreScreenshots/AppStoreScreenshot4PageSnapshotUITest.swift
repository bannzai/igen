...
/// App Store スクリーンショット 4 枚目 (AppStoreScreenshot4Page) を全言語で撮影する。
/// Attachment 名 `{クラス名}---{関数名}---{言語}---{index}` は scripts/generate_screenshots/organize_appstore_screenshots.sh が解析する
final class AppStoreScreenshot4PageSnapshotUITest: XCTestCase {
  ...
  let previewType = "AppStoreScreenshot4Page_Previews"
  let previewCount = 1
